...
class TransportDeliveryJournalPersistenceTests(unittest.TestCase):
    ...
    def test_v1_journal_migrates_without_losing_pending_outbox(self) -> None:
        ...
        outbox_due_plan = " ".join(
            str(row[3])
            for row in connection.execute(
                """
                EXPLAIN QUERY PLAN
                SELECT message_id FROM outbox INDEXED BY outbox_due
                WHERE expires_at > 0 AND attempts < max_attempts
                  AND next_attempt_at <= 10
                ORDER BY created_at
                """
            )
        )
        inbox_status_plan = " ".join(
            str(row[3])
            for row in connection.execute(
                """
                EXPLAIN QUERY PLAN
                SELECT message_id FROM inbox INDEXED BY inbox_status
        ...
        inbox_ack_plan = " ".join(
            ...
            for row in connection.execute(
                """
                EXPLAIN QUERY PLAN
                SELECT message_id FROM inbox INDEXED BY inbox_ack_due
                WHERE status = 'acked' AND ack_confirmed = 0
                  AND expires_at > 0 AND next_ack_at <= 10
        ...
        self.assertIn("outbox_due", outbox_due_plan)
        self.assertIn("inbox_status", inbox_status_plan)
        self.assertIn("inbox_ack_due", inbox_ack_plan)
        self.assertEqual(
            row,
