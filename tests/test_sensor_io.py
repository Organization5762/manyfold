...
class SensorIoTests(unittest.TestCase):
    ...
    def test_sensor_tag_metadata_is_key_sorted(self) -> None:
        ...
        self.assertEqual(tuple(tag.metadata), ("alpha", "zeta"))
    ...
    def test_retry_policy_sorts_unordered_exception_sets(self) -> None:
        manyfold = load_manyfold_package()

        policy = manyfold.SensorRetryPolicy(
            retry_on={RuntimeError, ValueError, LookupError},
        )

        self.assertEqual(policy.retry_on, (LookupError, RuntimeError, ValueError))
