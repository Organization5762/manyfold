...
class TopicDeliveryPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        resolved_inbox_items = (
            self.max_items
            if self.max_inbox_items is None
            else self.max_inbox_items
        )
        resolved_inbox_bytes = (
            self.max_bytes
            if self.max_inbox_bytes is None
            else self.max_inbox_bytes
        )
        _require_positive_integer(resolved_inbox_items, "max_inbox_items")
        _require_positive_integer(resolved_inbox_bytes, "max_inbox_bytes")
        object.__setattr__(self, "max_inbox_items", resolved_inbox_items)
        object.__setattr__(self, "max_inbox_bytes", resolved_inbox_bytes)
    ...
    def latest(
        ...
        *,
        ...
        return cls(
            ...
            max_inbox_items=max_inbox_items,
            max_inbox_bytes=max_bytes if max_inbox_bytes is None else max_inbox_bytes,
        )
