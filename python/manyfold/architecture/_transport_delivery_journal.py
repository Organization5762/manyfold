...
class _DeliveryJournal:
    ...
    def outbox_replay_records(self) -> tuple[_OutboxReplayRecord, ...]:
        with self._lock:
            self._require_open()
            rows = self._connection.execute(
                """
        ...
        return tuple(_OutboxReplayRecord(*row) for row in rows)
    ...
    def record_inbox(
        ...
        *,
        ...
        with self._transaction() as connection:
            ...
            usage = self._projected_usage(
                self._inbox_usage(connection, record.channel),
                size_bytes,
            )
            self._require_inbox_capacity(usage)
            self._require_inbox_topic_capacity(
                usage,
                topic=record.channel,
                item_limit=topic_item_limit,
                byte_limit=topic_byte_limit,
            )
            self._execute_write(
                connection,
                """
    ...
    def inbox_usage(self, topic: str) -> _OutboxUsage:
        with self._lock:
            self._require_open()
            return self._inbox_usage(self._connection, topic)
    ...
    def stats(self) -> _JournalStats:
        with self._lock:
            self._require_open()
            return self._stats(self._connection)

    def topic_stats(self, topic: str) -> _TopicStats:
        with self._lock:
            self._require_open()
            outbox = self._connection.execute(
                ...
                SELECT
                    COUNT(*),
                    COALESCE(
                        SUM(CASE WHEN semantics = 'append' THEN 1 ELSE 0 END),
                        0
                    ),
                    COALESCE(SUM(size_bytes), 0)
                FROM outbox WHERE topic = ?
                """,
                (topic,),
            ).fetchone()
        ...
        outbox_items = int(outbox[0])
        append_outbox_items = int(outbox[1])
        inbox_items = int(inbox[0])
        return _TopicStats(
            outbox_items,
            append_outbox_items,
            outbox_items - append_outbox_items,
            inbox_items,
            int(outbox[2]) + int(inbox[1]),
        )

    def validate_recovered_limits(
        self,
        *,
        topic_policies: dict[str, _RecoveredTopicPolicy],
        default_topic_policy: _RecoveredTopicPolicy,
    ) -> None:
        """Fail closed when a recovered journal exceeds current retention policy."""
        with self._lock:
            self._require_open()
            stats = self._stats(self._connection)
            if stats.outbox_items > self._max_outbox_items:
                raise _JournalFull(
                    f"recovered outbox has {stats.outbox_items} items; "
                    f"current limit is {self._max_outbox_items}"
                )
            inbox_items = stats.pending_inbox_items + stats.acked_inbox_items
            if inbox_items > self._max_inbox_items:
                raise _JournalFull(
                    f"recovered inbox has {inbox_items} items; "
                    f"current limit is {self._max_inbox_items}"
                )
            if stats.logical_bytes > self._max_storage_bytes:
                raise _JournalFull(
                    "recovered delivery journal has "
                    f"{stats.logical_bytes} logical bytes; current limit is "
                    f"{self._max_storage_bytes}"
                )
            for topic, semantics, item_count, byte_count in self._connection.execute(
                """
                SELECT topic, semantics, COUNT(*), COALESCE(SUM(size_bytes), 0)
                FROM outbox
                GROUP BY topic, semantics
                """
            ):
                topic_name = str(topic)
                policy = topic_policies.get(
                    topic_name,
                    default_topic_policy,
                )
                if str(semantics) != policy.semantics:
                    raise _JournalFull(
                        f"recovered outbox topic {topic_name!r} has "
                        f"{str(semantics)!r} rows; current policy requires "
                        f"{policy.semantics!r}"
                    )
                ...
                if int(item_count) > item_limit:
                    raise _JournalFull(
                        f"recovered outbox topic {topic_name!r} has "
                        f"{int(item_count)} items; current limit is {item_limit}"
                    )
                if int(byte_count) > byte_limit:
                    raise _JournalFull(
                        f"recovered outbox topic {topic_name!r} has "
                        f"{int(byte_count)} logical bytes; current limit is "
                        f"{byte_limit}"
                    )
            for topic, source_key, item_count in self._connection.execute(
                """
                SELECT topic, source_key, COUNT(*)
                FROM outbox
                WHERE semantics = 'latest'
                GROUP BY topic, source_key
                """
            ):
                topic_name = str(topic)
                policy = topic_policies.get(
                ...
                if not policy.latest_per_source and source_key not in (None, ""):
                    raise _JournalFull(
                        f"recovered latest topic {topic_name!r} has source "
                        "keys but current policy has one shared latest slot"
                    )
                if policy.latest_per_source and not source_key:
                    raise _JournalFull(
                        f"recovered latest topic {topic_name!r} has no "
                        "source key but current policy requires source slots"
                    )
                if int(item_count) > 1:
                    source_detail = "" if source_key is None else f" {source_key!r}"
                    raise _JournalFull(
                        f"recovered latest topic {topic_name!r} source"
                        f"{source_detail} has {int(item_count)} retained rows; "
                        "current policy allows one"
                    )
            for topic, source_count in self._connection.execute(
                """
                SELECT topic, COUNT(DISTINCT source_key)
                FROM outbox
                WHERE semantics = 'latest'
                GROUP BY topic
                """
                ...
                if policy.latest_per_source and int(source_count) > policy.max_items:
                    raise _JournalFull(
                        f"recovered latest topic {topic_name!r} has "
                        f"{int(source_count)} source slots; current limit is "
                        f"{policy.max_items}"
                    )
            for topic, item_count, byte_count in self._connection.execute(
                """
                SELECT channel, COUNT(*), COALESCE(SUM(size_bytes), 0)
                FROM inbox
                GROUP BY channel
                """
            ):
                ...
                if int(item_count) > policy.max_inbox_items:
                    raise _JournalFull(
                        f"recovered inbox topic {topic_name!r} has "
                        f"{int(item_count)} items; current limit is "
                        f"{policy.max_inbox_items}"
                    )
                if int(byte_count) > policy.max_inbox_bytes:
                    raise _JournalFull(
                        f"recovered inbox topic {topic_name!r} has "
                        f"{int(byte_count)} logical bytes; current limit is "
                        f"{policy.max_inbox_bytes}"
                    )
    ...
    def _require_inbox_topic_capacity(
        self,
        usage: _OutboxUsage,
        *,
        topic: str,
        ...
        if usage.topic_items > item_limit:
            raise _JournalFull(
                f"inbox topic {topic!r} item limit {item_limit} is full",
                capacity=usage,
            )
        if usage.topic_bytes > byte_limit:
            raise _JournalFull(
                f"inbox topic {topic!r} byte limit {byte_limit} "
                "would be exceeded",
                capacity=usage,
            )

    def _require_inbox_capacity(self, usage: _OutboxUsage) -> None:
        if usage.items > self._max_inbox_items:
            raise _JournalFull(
                f"inbox item limit {self._max_inbox_items} is full",
                capacity=usage,
            )
        if usage.logical_bytes > self._max_storage_bytes:
            raise _JournalFull(
    ...
    def _inbox_usage(
        ...
        return _OutboxUsage(
            items=int(row[0]),
            logical_bytes=int(row[1]) + outbox_bytes,
            topic_items=int(topic_row[0]),
            topic_bytes=int(topic_row[1]),
        )

    def _stats(self, connection: sqlite3.Connection) -> _JournalStats:
        outbox_items = self._count(connection, "outbox")
        ...
        return _JournalStats(
            ...
            pending,
            acked,
            self._logical_bytes(connection),
        )
