...
_LEGOS = (
    ...
    Lego(
        "OrderingPolicy", "ordering", "policy", "Describes event ordering guarantees."
    ),
    Lego(
        "OverflowPolicy", "flow", "policy", "Describes behavior when capacity is full."
    ),
    Lego(
        "RetentionPolicy", "persistence", "policy", "Describes how long state is kept."
    ),
    Lego(
        "DurabilityPolicy", "persistence", "policy", "Describes persistence strength."
    ),
    Lego(
        "ConsistencyPolicy", "coordination", "policy", "Describes read/write agreement."
    ),
    ...
    Lego(
        ...
        "property",
        "property",
        "Externally visible effect happens once.",
    ),
    ...
    Lego(
        ...
        "communication",
        "local",
        "Closed event with identity and payload metadata.",
        ("Schema", "MessageId", "SequenceNumber"),
    ),
    Lego(
        "Clock",
        "time",
    ...
    Lego(
        ...
        "time",
        "local",
        "Fires after a duration or at a deadline.",
    ...
    Lego(
        ...
        "compute",
        "local",
        "Stops work when cancellation is requested.",
    ...
    Lego(
        ...
        "recovery",
        "local",
        "Retries local work according to policy.",
    ...
    Lego(
        ...
        "ordering",
        "local",
        "Assigns monotonic local sequence numbers.",
        ("SequenceNumber",),
    ),
    Lego(
        "Buffer", "flow", "local", "Temporarily stores items in memory.", ("Capacity",)
    ),
    Lego(
        "RingBuffer",
        "flow",
        "local",
        "Bounded cyclic staging buffer.",
        ("Buffer", "OverflowPolicy", "OrderingPolicy"),
    ),
    Lego(
        "Queue",
        "handoff",
        "local",
        "Orders items for later consumption.",
        ("Buffer", "OrderingPolicy"),
    ),
    Lego(
        "BoundedQueue",
        "flow",
        "local",
        "Queue with explicit capacity.",
        ("Queue", "Capacity", "OverflowPolicy"),
    ),
    Lego(
        "TokenBucket",
        "flow",
        "local",
        "Tracks rate and burst credits.",
        ("Clock", "RatePolicy"),
    ),
    Lego(
        "Semaphore",
        "flow",
        "local",
        "Bounds concurrent holders.",
        ("Capacity", "ConcurrencyPolicy"),
    ),
    Lego(
        "RateLimiter",
        "flow",
        "local",
        "Allows, delays, or rejects work by rate.",
        ("TokenBucket", "Cost"),
    ),
    Lego(
        "Watchdog",
        "observability",
        "local",
        "Emits status when expected events stop.",
        ("Timer", "Status"),
    ),
    Lego(
        "Sampler",
        "compute",
        "local",
        "Selects values according to a sampling policy.",
        ("Clock", "OrderingPolicy"),
    ),
    Lego(
        "ChangeFilter",
        "flow",
        "local",
        "Suppresses unchanged values.",
        ("Deterministic",),
    ),
    Lego(
        "ThresholdFilter",
        "flow",
        "local",
        "Suppresses changes below a numeric threshold.",
        ("ChangeFilter",),
    ),
    Lego(
        ...
        "communication",
        "local",
        "Reassembles delimited byte or text messages.",
        ("Buffer", "Bytes"),
    ),
    Lego(
        "JsonEventDecoder",
        "communication",
        "local",
        "Decodes JSON messages into sensor events.",
        ("Schema", "SensorEvent", "SequenceCounter"),
    ),
    Lego(
        "DoubleBuffer",
        "flow",
        "local",
        "Alternates active and ready buffers for bursty capture.",
        ("Buffer", "Capacity"),
    ),
    Lego(
        "FrameAssembler",
        "communication",
        "local",
        "Aggregates samples into complete frames.",
        ("SequenceNumber",),
    ),
    Lego(
        "XorChecksum",
        "data",
        "local",
        "Computes a small XOR checksum over bytes.",
        ("Bytes", "Checksum"),
    ),
    Lego("RouteRef", "communication", "local", "Typed route identity.", ("Schema",)),
    Lego(
        "PortRef", "communication", "local", "Typed endpoint identity.", ("RouteRef",)
    ),
    Lego(
        "Graph",
        "communication",
        "local",
        "Connects typed routes and operators.",
    ...
    Lego(
        ...
        "handoff",
        "local",
        "Bounded graph-visible async boundary.",
        ("BoundedQueue", "RouteRef", "Ack"),
    ),
    Lego(
        "Pipe",
        "communication",
        "local",
        "Moves events between source and route.",
        ("RouteRef", "Publish", "Subscribe"),
    ),
    Lego(
        "Bridge",
        "communication",
        "local",
        "Connects routes across an adapter boundary.",
        ("Pipe", "Schema"),
    ),
    Lego(
        "PubSub",
    ...
    Lego(
        ...
        "persistence",
        "durable",
        "Durably stores bytes by key.",
        ("Key", "Bytes", "Read", "Write", "Delete"),
    ),
    Lego(
        "Keyspace",
        "persistence",
        "durable",
        "Scoped structured prefix over a byte store.",
        ("ByteStore", "Key"),
    ),
    Lego(
        "ObjectStore",
        "persistence",
        "durable",
        "Durably stores large objects.",
        ("ByteStore", "RetentionPolicy"),
    ),
    Lego(
        "BlobStore",
        "persistence",
        "durable",
        "Stores opaque byte blobs.",
    ...
    Lego(
        ...
        "persistence",
        "durable",
        "Durable single value.",
        ("ByteStore", "DurabilityPolicy"),
    ),
    Lego(
        "VersionedRegister",
        "persistence",
        "durable",
        "Durable value with compare-and-swap version.",
        ("DurableCell", "Version", "CompareAndSwap"),
    ),
    Lego(
        "WriteAheadLog",
        "persistence",
        "durable",
        "Appends intent before applying state.",
        ("Keyspace", "Append", "SequenceNumber", "DurabilityPolicy"),
    ),
    Lego(
        "EventLog",
        "persistence",
        "durable",
        "Typed append-only durable event history.",
        ("WriteAheadLog", "Envelope", "Offset"),
    ),
    Lego(
        "SnapshotStore",
        "persistence",
        "durable",
        "Durable latest state image.",
        ("Keyspace", "Schema"),
    ),
    Lego(
        "CheckpointStore",
        ...
        ("SnapshotStore", "Offset"),
    ),
    Lego(
        "MetadataStore",
        "configuration",
        "durable",
        "Durable control-plane facts.",
        ("Keyspace", "VersionedRegister"),
    ),
    Lego(
        "IdempotencyStore",
        "recovery",
        "durable",
        "Remembers completed request IDs.",
        ("Keyspace", "RequestId"),
    ),
    Lego(
        "DeduplicationStore",
    ...
    Lego(
        ...
        "replication",
        "durable",
        "Remembers deletes across repair.",
        ("Keyspace", "RetentionPolicy"),
    ),
    Lego(
        "MaterializedView",
        "persistence",
        "durable",
        "Queryable projection from event history.",
    ...
    Lego(
        ...
        "handoff",
        "local",
        "Tracks ack, nack, and visibility deadlines.",
        ("Ack", "Nack", "Deadline", "IdempotencyStore"),
    ),
    Lego(
        "VisibilityDeadline",
        "handoff",
        "local",
        "Time boundary for a claimed item.",
        ("Deadline", "TTL"),
    ),
    Lego(
        "DurableQueue",
        "handoff",
        "durable",
        "Durably accepts work and later delivers it.",
        (
            "EventLog",
            "CheckpointStore",
            "AckTracker",
    ...
    Lego(
        ...
        ("Queue", "Priority", "PriorityPolicy"),
    ),
    Lego(
        "DelayQueue",
        "handoff",
        "local",
        "Queue that releases items after time.",
        ("Queue", "Timer"),
    ),
    Lego(
        "WorkQueue",
        "handoff",
        "durable",
        "Queue specialized for executable work.",
        ("DurableQueue", "IdempotencyStore"),
    ),
    Lego(
        "RetryQueue",
        "recovery",
        "durable",
        "Durably stores work waiting for retry.",
        ("DurableQueue", "Backoff"),
    ),
    Lego(
        "DeadLetterQueue",
        "recovery",
        "durable",
        "Stores work that cannot be completed.",
        ("DurableQueue", "Error"),
    ),
    Lego(
        "Outbox",
        "handoff",
        "durable",
        "Durably records outgoing side effects.",
    ...
    Lego(
        ...
        "handoff",
        "durable",
        "Durably records incoming side effects.",
        ("EventLog", "DeduplicationStore"),
    ),
    Lego(
        "ConsumerGroup",
        "handoff",
        "distributed",
        "Shares queue partitions across consumers.",
        ("Membership", "CheckpointStore", "PartitionMap"),
    ),
    # Distributed.
    Lego(
        "Heartbeat",
    ...
    Lego(
        ...
        "membership",
        "distributed",
        "Suspects nodes after missed heartbeats.",
        ("Heartbeat", "Timeout"),
    ),
    Lego(
        "Membership",
        "membership",
        "distributed",
        "Tracks known live or suspected nodes.",
        ("NodeId", "FailureDetector"),
    ),
    Lego(
        "FencingToken",
        "coordination",
        "distributed",
        "Monotonic token preventing stale owners.",
        ("SequenceNumber",),
    ),
    Lego(
        "Lease",
        "coordination",
        "distributed",
        "Grants temporary ownership.",
        ("Deadline", "Renew", "Release", "FencingToken", "VersionedRegister"),
    ),
    Lego(
        "Quorum",
        "coordination",
        "distributed",
        "Requires enough members to agree.",
        ("Membership", "ConsistencyPolicy"),
    ),
    Lego(
        "LeaderElection",
        "coordination",
        "distributed",
        "Selects one leader for a term.",
        ("Membership", "Quorum", "Term", "Timer"),
    ),
    Lego(
        "Transport",
        "communication",
    ...
    Lego(
        ...
        "replication",
        "distributed",
        "Copies an ordered log across replicas.",
        ("Consensus", "EventLog"),
    ),
    Lego(
        "ReplicaSet",
        "replication",
        "distributed",
        "Names a group of replicas.",
        ("Membership", "ReplicationPolicy"),
    ),
    Lego(
        "Replicator",
        "replication",
        "distributed",
        "Copies state or work between replicas.",
        ("EventLog", "ReplicaSet", "CheckpointStore"),
    ),
    Lego(
        "AntiEntropyRepair",
        "replication",
        "distributed",
        "Repairs divergent replicas.",
        ("Replicator", "Checksum", "ConflictPolicy"),
    ),
    Lego(
        "ServiceRegistry",
        "routing",
        "distributed",
        "Maps service names to endpoints.",
        ("MetadataStore", "ServiceId"),
    ),
    Lego(
        "Router",
        "routing",
        "distributed",
        "Chooses a destination for a key or route.",
        ("RouteRef", "RoutingPolicy", "MetadataStore"),
    ),
    Lego(
        "LoadBalancer",
        "routing",
        "distributed",
        "Spreads requests across healthy endpoints.",
        ("Router", "HealthStatus"),
    ),
    Lego(
        "PartitionMap",
    ...
    Lego(
        ...
        "distributed",
        "Maps keys to shards.",
        ("PartitionMap", "PartitionPolicy"),
    ),
    Lego(
        "ConsistentHashRing",
        "partitioning",
        "distributed",
        "Maps keys to owners with low churn.",
        ("ShardMap", "Hash"),
    ),
    Lego(
        "MigrationFence",
        "partitioning",
        "distributed",
        "Prevents stale writes during movement.",
        ("FencingToken", "Epoch"),
    ),
    Lego(
        "Rebalancer",
        "partitioning",
        "distributed",
        "Moves partitions to improve balance.",
        ("ShardMap", "Lease", "CheckpointStore", "MigrationFence"),
    ),
    ...
    Lego(
        ...
        "observability",
        "application",
        "Numeric operational measurement.",
        ("Timestamp",),
    ),
    Lego(
        "TraceSpan",
    ...
    Lego(
        ...
        "observability",
        "application",
        "Durable security or control event.",
    ...
    Lego(
        ...
        ("Envelope", "CausalityId", "CorrelationId"),
    ),
    Lego(
        "Worker",
        "compute",
        ...
        ("WorkQueue", "Lease", "IdempotencyStore"),
    ),
    Lego(
        "WorkerPool",
        "compute",
        ...
        ("Worker", "Membership", "FlowControl"),
    ),
    Lego(
        "WorkflowStep",
        "workflow",
        "application",
        "One durable unit in a workflow.",
        ("EventLog", "RetryLoop"),
    ),
    Lego(
        "Workflow",
        "workflow",
        "application",
        "Durably orchestrates multi-step work.",
        (
            "EventLog",
            "DurableQueue",
            "CheckpointStore",
    ...
    Lego(
        ...
        "recovery",
        "application",
        "Undo or counteract a completed effect.",
        ("IdempotencyStore", "EventLog"),
    ),
    Lego(
        "SensorSample",
        "compute",
        "application",
        "Normalized local sensor reading.",
        ("Timestamp", "SequenceNumber", "Status"),
    ),
    Lego(
        "SensorTag",
        "identity",
        "application",
        "Metadata tag for a sensor or peripheral.",
        ("Key",),
    ),
    Lego(
        "SensorLocation",
        "identity",
        "application",
        "Physical coordinate for a sensor.",
        ("Timestamp",),
    ),
    Lego(
        "SensorIdentity",
        "identity",
        "application",
        "Identity, tags, location, and group for a sensor.",
        ("SensorTag", "SensorLocation"),
    ),
    Lego(
        "SensorEvent",
        "communication",
        "application",
        "Normalized event payload for sensor and peripheral streams.",
        ("SensorIdentity", "Timestamp", "SequenceNumber"),
    ),
    Lego(
        "SensorDebugTap",
        "observability",
        "application",
        "Records and broadcasts sensor debug emissions.",
        ("SensorEvent", "Clock"),
    ),
    Lego(
        "LocalSensorSource",
        "compute",
        "application",
        "Polls a local sensor and publishes samples.",
        ("Clock", "RetryLoop", "SequenceCounter", "SensorSample", "RouteRef"),
    ),
    Lego(
        "ReactiveSensorSource",
        "compute",
        "application",
        "Publishes observable sensor emissions to a route.",
        ("Clock", "SequenceCounter", "RouteRef", "SensorSample"),
    ),
    Lego(
        "PeripheralAdapter",
        "communication",
        "application",
        "Adapts Heart-style peripheral envelopes to routes.",
        ("SensorIdentity", "SensorEvent", "RouteRef"),
    ),
    Lego(
        "DuplexSensorPeripheral",
        "communication",
        "application",
        "Adapts peripherals with both output events and control input.",
    ...
    Lego(
        ...
        "flow",
        "application",
        "Buffers sensor samples and emits by demand.",
        ("RingBuffer", "Clock", "Graph"),
    ),
    Lego(
        "SensorHealthWatchdog",
        "observability",
        "application",
        "Reports stale or failing sensor input.",
        ("Clock", "Watchdog", "HealthStatus"),
    ),
    Lego(
        "LocalDurableSpool",
        "recovery",
        "application",
        "File-backed local sample retention.",
        ("EventLog", "Keyspace"),
    ),
    Lego(
        "DesiredState",
        "configuration",
        "application",
        "Target configuration or topology.",
        ("Version",),
    ),
    Lego(
        "ObservedState",
        "configuration",
        "application",
        "Measured current configuration or topology.",
        ("Version",),
    ),
    Lego(
        "Reconciler",
        "control-plane",
        "application",
        "Moves observed state toward desired state.",
        ("DesiredState", "ObservedState", "RetryLoop", "EventLog"),
    ),
    Lego(
        "FeatureFlag",
        "configuration",
        "application",
        "Runtime-switchable behavior flag.",
    ...
    Lego(
        ...
        "control-plane",
        "application",
        "Rolls out changes with health gates.",
        ("Reconciler", "HealthStatus", "MetricSample"),
    ),
    Lego(
        "KillSwitch",
        "control-plane",
        ...
        ("FeatureFlag", "AuthPolicy"),
    ),
    Lego(
        "FlowControl",
        "flow",
        "application",
        "Coordinates backpressure, admission, and limits.",
        ("RateLimiter", "Semaphore", "BoundedQueue"),
    ),
)
...
_DEPENDENCY_CLOSURE_BY_NAME = {
    lego.name: _dependency_closure_for(lego.name) for lego in _LEGOS
}
